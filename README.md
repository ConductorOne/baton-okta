...
Generated passwords use the SDK's supported 8–64 character range. Older versions silently applied `min(8, requested_length)`, so a request above 64 could create an eight-character password. It now fails with `InvalidArgument` before provider writes rather than silently downgrading the request. Review affected policies before upgrading. This structural validation is independent of `--strict-account-creation`, which governs legacy mandatory-change composition.
