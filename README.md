...
- Configuring through the C1 connector form instead of the CLI? The connector declares **Okta Private Key** as a multiline field, so where the form renders it as a text area the PEM goes in as-is. On a C1 release that predates that support the field is a single-line input: write each line break as a literal `\n`, or a pasted multi-line PEM fails with `no PEM block found`. The escaped form keeps working either way. See [`docs/connector.mdx`](docs/connector.mdx).
