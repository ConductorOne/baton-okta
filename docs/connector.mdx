...
The tested SDK baseline for these user-read and partial-create contracts is `baton-sdk v0.29.0`, the exact version pinned in `go.mod`. Its `syncer.getResourceFromConnector` explicitly treats `NotFound` as a skip; the connector's SDK/local-gRPC fixture preserves both that status and the filter qualifier. Older SDK versions and different hosted event-reconciliation implementations are not certified by this PR without equivalent verification.
</Note>
