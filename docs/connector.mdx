...
| :--- | :--- | :--- | :--- |
| **Password Change Required on Login** | `True` / `False` | Off | Requires a password change on the next login for a supplied or generated password. |
| **Create Inactive** | `True` / `False` | Off | Creates the user in Okta as staged (inactive) without activating them. |
| **Send Activation Email** | `True` / `False` | `True` | When set to `False`, activates the user without sending Okta's activation email. |
| **Provider Type** | `OKTA` / `FEDERATION` | Okta's default (local) provider | Set to `FEDERATION` to create a federated user with no local Okta password. |
| **Additional Attributes** | Map of Okta profile keys | — | Extra Okta profile attributes to set on the user. |

<Note>
`FEDERATION` cannot be combined with a supplied or generated password. A profile or SDK mandatory-password-change request requires a password and cannot be combined with inactive creation or suppressed activation email. All invalid combinations fail before any provider write.
</Note>

<Note>
Generated passwords honor the requested SDK length and constraints and are returned only through the SDK protected-result path. Supplied passwords are not returned. Invalid SDK options fail before provider writes; SDK generation and decryption errors are returned to the caller.
</Note>

<Note>
**Creation status.** A successful create reports the provider Resource and its actual status; it does not promise sign-in readiness. With **Send Activation Email** set to `False`, the connector activates the created ID and reads it back. If activation or that read cannot be confirmed, the result is **Action Required** with the known created resource and protected generated credential material. A successful read returns **Success**, including when Okta still reports `STAGED`.
</Note>

<Note>
Targeted user `Get` bypasses the SDK GET cache. Resource profiles retain the raw Okta status; they do not add connector lifecycle transition or timestamp fields. Filtered user reads return gRPC `NotFound` with `google.rpc.ErrorInfo` reason `RESOURCE_FILTERED`; malformed provider successes remain errors.
</Note>
### Connector actions
