...
That escaped form keeps working either way — the connector expands a literal `\n` back into a line break before parsing — so a key entered this way needs no migration.
