...
| **Send Activation Email** | Toggle | On | When turned off, activates the user without sending Okta's activation email. Use this when C1 (or another system) owns user communication. |
...
Navigate to the **Okta API Scopes** tab and grant the scopes required for your use case:
   ...
   - `okta.devices.read` (required when **Sync devices** is enabled)
...
<Step>
**Optional.** If desired, click the checkbox to **Sync devices** to include Okta devices in sync. This is off by default. Devices are synced read-only; the connector does not manage device assignments.
</Step>
<Step>
...
stringData:
  ...
  # Optional: sync Okta devices (off by default)
  # BATON_SYNC_DEVICES: true
```
