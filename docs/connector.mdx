...
| :--- | :--- | :--- | :--- |
| **Password Change Required on Login** | `True` / `False` | Off | When creating an account with a supplied or generated password, requires password replacement at first login only on a supported active-create path. Unsupported inactive combinations fail before creation. |
...
<Note>
**Provider Type** `FEDERATION` cannot be combined with a supplied or generated password: federated users do not have a local Okta password. **Send Activation Email** `False` cannot be combined with mandatory password change when creating with a password. **Create Inactive** `True` also cannot be combined with that requirement. The connector rejects these unsupported combinations before creating, activating, or expiring a user. The SDK credential option `force_change_at_next_login` cannot be silently ignored. With the legacy no-password profile option, Password Change Required on Login has no effect; use a separately authorized enrollment flow.
</Note>

<Warning>
**Mandatory employee takeover is not established by staged activation.** Earlier versions silently ignored Password Change Required on Login during inactive creation; that combination now fails closed. Valid inactive creation with a protected supplied password remains supported, and `enable_user` still activates a bound `STAGED` account or unsuspends a `SUSPENDED` one without rotating the password.
...
<Note>
**Fresh state and correlation.** Targeted user `Get` bypasses the SDK GET cache and preserves `transitioningToStatus`. It adds `c1_okta_fresh_observation` and `c1_okta_observed_at`; ordinary sync/create snapshots are not stamped fresh merely because they were processed. Resource profiles preserve raw status plus present `c1_okta_transitioning_to_status`, `c1_okta_status_changed_at`, `c1_okta_password_changed_at`, and `c1_okta_last_updated_at` facts. Missing fields remain unknown, and these connector-owned metadata fields cannot be supplied by custom profile attributes.

A filtered user is an explicit permission/filter error, not absence. Missing/incomplete or mismatched identity data is not adopted. Use a targeted read to reconcile the retained provider ID instead of creating a replacement account or assuming a full sync will resolve it. Provider timestamps are observations, not proof that an employee changed a credential or completed enrollment.
</Note>
