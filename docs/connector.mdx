...
<Note>
**Provider Type** `FEDERATION` cannot be combined with a supplied or generated password. **Strict account creation validation** (`--strict-account-creation`, default `false`) preserves legacy configurations while providing a fail-before-write path for generated-password bootstrap. Supplied-password requests are always strict, even when this flag is off. Strict supplied/generated requests reject inactive creation plus mandatory change, and strict validation rejects SDK force-change without a password. The pre-existing random-password conflict between suppressed activation email and the profile's mandatory-change flag remains an error on non-inactive creation; legacy inactive precedence is preserved.
</Note>
