...
<Note>
**Private key format.** Paste the PEM-encoded key into **Okta Private Key**, line breaks and all.

If that field only accepts a single line, enter the key on one line instead, writing each line break as a literal `\n` — a multi-line PEM pasted into a single-line field loses its line breaks and is rejected with `oktaauth: parse private key: no PEM block found`:
...
The escaped form is always accepted — the connector expands a literal `\n` into a line break before parsing — so a key entered that way never needs changing.

Running the connector self-hosted, pass the key unchanged: `BATON_OKTA_PRIVATE_KEY` and `--okta-private-key` accept the multi-line PEM as it is.
...
Enter your credentials:
   ...
   - For **OAuth 2.0 Private Key**: enter your **Okta Client ID** and **Okta Private Key ID**, then paste your RSA private key into **Okta Private Key**. If that field only accepts a single line, write each line break as `\n` — see **Private key format** above.
</Step>
<Step>
