...
**Upgrade compatibility:** older versions applied `min(8, requested_length)`, so a policy requesting more than 64 characters could report success with only an eight-character password. Such requests now fail with `InvalidArgument` before any provider write instead of silently downgrading the requested length. Review affected policies before upgrading and use a length supported by the SDK. This structural credential validation applies regardless of `--strict-account-creation`; that flag controls legacy mandatory-change option composition, not password-length validation.
</Note>
...
After verifying the returned provider identity is the account bound to the intended request, use `enable_user` to request activation. Do not activate an unrelated duplicate account or treat this as evidence that the first-login takeover gate has been met.
</Note>
