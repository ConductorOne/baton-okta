...
The Okta connector supports account deprovisioning. When a user account is deprovisioned through C1, the connector deactivates and then deletes the user in Okta. The deactivation step transitions the user to the DEPROVISIONED state, and the deletion step permanently removes the user. This requires an API token or OAuth app with `okta.users.manage` scope.
