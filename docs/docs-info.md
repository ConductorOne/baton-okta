...
2. Can the connector provision any resources? If so, which ones?
   ...
   - **update_profile** — Partial update of an existing user's profile via `POST /api/v1/users/{id}` (Okta's partial-update semantics; only the fields provided are changed). Manual/UX invocation with individual typed fields. `user_id` is a `ResourceIdField` restricted to the `user` resource type — the caller must pass `{"resource_type_id":"user","resource_id":"00u…"}`, which is what a workflow step's resource selector produces.
   - **update_user** — Same underlying `POST /api/v1/users/{id}` call, but takes a `user_profile` JSON object instead of typed fields and accepts any Okta profile attribute (no curated allowlist). `user_id` here is a plain `StringField` — the caller must pass the raw Okta ID directly. This is the global-action shape C1's automated profile-push pipeline requires — a resource-scoped-only action does not satisfy that lookup.
   - The `user_id` shape is what actually separates the two, not just the typed-fields/JSON-blob or manual/push-pipeline framing above: `pkg/actions/args.go`'s `GetStringArg` and `GetResourceIDArg` both type-assert on the argument's `structpb` kind, so the SDK does not coerce between shapes. A caller holding one `user_id` shape cannot invoke the other action — passing the wrong shape fails with `InvalidArgument` before any Okta call is made. Both actions declare the same `ACTION_TYPE_ACCOUNT_UPDATE_PROFILE` action type, so action type does not distinguish them.
...
2. For each item:
   ...
   * **Does it need specific scopes/permissions?**
     ...
     - Event feed: `okta.logs.read` (System Log; required or `list_events` fails every run)
...
- Mandatory change from either the profile flag or SDK `force_change_at_next_login` conflicts with `create_inactive=true` and `send_activation_email=false` on password-bearing requests. These combinations fail before create/activate/expire; the requirement is never silently dropped.
- `create_inactive=true` otherwise wins over `send_activation_email`. The legacy profile flag remains inert on no-password paths; an explicit SDK force-change without a password is rejected.
...
An unconfirmed activation retains the created resource and generated credential material in `ActionRequiredResult`. The resource is labeled as the creation snapshot, not proof of current staged state. Acknowledged activation uses the fresh resource `Get` path; an unavailable/mismatched read retains the original identity, while a still-staged observation returns `InProgressResult`. No unknown create or activation is replayed inside the connector.

Existing accounts are never activated, renamed, or given new credentials by duplicate-create handling. Verify the binding before explicitly using `enable_user`. Targeted Get carries `c1_okta_observed_at` and `c1_okta_fresh_observation`; raw status and present transition/provider timestamps are separate facts. These metadata fields are reserved and cannot be forged by custom profile attributes. Provider timestamps do not establish employee takeover.
