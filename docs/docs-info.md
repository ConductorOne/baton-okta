...
Lifecycle races are reconciled using structured state, not vendor error text. If deactivation is rejected while another actor is changing the same user, the connector performs one fresh status read. It proceeds only when Okta now reports `DEPROVISIONED`, treats absence as success only for a delete operation, and otherwise preserves the original mutation error. A missing user is idempotent success for delete operations; the deactivate-only action keeps missing as `NotFound` because its requested result is a surviving `DEPROVISIONED` user.
