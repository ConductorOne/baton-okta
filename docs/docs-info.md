...
Lifecycle races are reconciled using structured state, not vendor error text. If deactivation is rejected while another actor is changing the same user, the connector performs one fresh status read. It proceeds only when Okta now reports `DEPROVISIONED`, treats absence as success only for a delete operation, and otherwise preserves the original mutation error. The Okta SDK classifies every 2xx as transport success, but permanent deletion requires `204 No Content`: another 2xx triggers a fresh status read and, once deactivation is complete, the required second DELETE. The connector returns a retryable error instead of claiming success if 204 is still not confirmed.
