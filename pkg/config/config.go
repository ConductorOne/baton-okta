...
var (
	...
	cacheTTI              = field.IntField("cache-tti", field.WithDescription("Response cache cleanup interval in seconds"), field.WithDefaultValue(60), field.WithStructFieldName("CacheTTI"))
	cacheTTL              = field.IntField("cache-ttl", field.WithDescription("Response cache time to live in seconds"), field.WithDefaultValue(300), field.WithStructFieldName("CacheTTL"))
	...
	awsIdentityCenterMode = field.BoolField("aws-identity-center-mode",
		field.WithDescription("Whether to run in AWS Identity center mode or not. In AWS mode, only samlRoles for groups and the users assigned to groups are synced"), field.WithStructFieldName("AWSMode"))
